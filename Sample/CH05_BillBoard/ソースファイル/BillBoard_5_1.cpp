...
float CheckGroundHeight( MY_PLAYER *pPlayer )									// 地面の高さチェック
{
	...
	if ( ( fBlockRes_x + fBlockRes_z ) < 1.0f ) {
		...
		nInIndex = 0;
		v3Vec1 = XMFLOAT3( 0.0f,        fGrad_z, BLOCK_WIDTH );
		v3Vec2 = XMFLOAT3( BLOCK_WIDTH, fGrad_x, 0.0f );
	}
	else {
	...
	pPlayer->v3Normal = Cross( &v3Vec1, &v3Vec2 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "BillBoard_5_1" ),
