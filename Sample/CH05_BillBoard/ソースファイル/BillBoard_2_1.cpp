...
#define BILLBOARD_NUM				10					// ビルボードの数
...
XMMATRIX MakeBillboardMatrix( XMMATRIX *pmatView )		// ビルボード行列の生成(原点限定)
{
	...
	matBill._41 =          0.0f;  matBill._42 =          0.0f;  matBill._43 =          0.0f;  matBill._44 = 1.0f;
...
{
	// プレイヤー1
	Player_1.v3Pos = XMFLOAT3( 0.0f, -3.0f, 0.0f );
...
int MovePlayer( void )									// 視点の移動
{
	...
	fEyeAngle = 2.0f * PI / 6000.0f * ( timeGetTime() % 6000 );
...
HRESULT InitGeometry( void )
{
	...
	// モデル作成
	// ビルボード
	int nVertexIndex, nIndexIndex;
	float			x, z;
	float			fAngle, fAngleDelta;
	fAngle = 0.0f;
	fAngleDelta = 2.0f * PI / BILLBOARD_NUM;
	nVertexIndex = 0;
	nIndexIndex = 0;
	for ( i = 0; i < BILLBOARD_NUM; i++ ) {
		x = 3.0f * cosf( fAngle );
		z = 3.0f * sinf( fAngle );
		g_cvVertices[g_nVertexNum + nVertexIndex    ].v4Pos = XMFLOAT4( x - BILLBOARD_SIZE / 2, BILLBOARD_SIZE / 2, z, 1.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex    ].v2UV = XMFLOAT2( 0.0f, 0.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 1].v4Pos = XMFLOAT4( x + BILLBOARD_SIZE / 2, BILLBOARD_SIZE / 2, z, 1.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 1].v2UV = XMFLOAT2( 1.0f, 0.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 2].v4Pos = XMFLOAT4( x - BILLBOARD_SIZE / 2, -BILLBOARD_SIZE / 2, z, 1.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 2].v2UV = XMFLOAT2( 0.0f, 1.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 3].v4Pos = XMFLOAT4( x + BILLBOARD_SIZE / 2, -BILLBOARD_SIZE / 2, z, 1.0f );
		g_cvVertices[g_nVertexNum + nVertexIndex + 3].v2UV = XMFLOAT2( 1.0f, 1.0f );
		g_wIndices[g_nIndexNum + nIndexIndex    ] = nVertexIndex + 0;
		g_wIndices[g_nIndexNum + nIndexIndex + 1] = nVertexIndex + 2;
		g_wIndices[g_nIndexNum + nIndexIndex + 2] = nVertexIndex + 1;
		g_wIndices[g_nIndexNum + nIndexIndex + 3] = nVertexIndex + 1;
		g_wIndices[g_nIndexNum + nIndexIndex + 4] = nVertexIndex + 2;
		g_wIndices[g_nIndexNum + nIndexIndex + 5] = nVertexIndex + 3;
		nVertexIndex += 4;
		nIndexIndex += 6;
		fAngle += fAngleDelta;
	}
	g_mmBillboard.nVertexPos = g_nVertexNum;
	g_mmBillboard.nVertexNum = nVertexIndex;
	g_mmBillboard.nIndexPos = g_nIndexNum;
	g_mmBillboard.nIndexNum = nIndexIndex;
	g_nVertexNum += nVertexIndex;
	g_nIndexNum += nIndexIndex;
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		...
		for ( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ ) {
			...
			rsq = x * x + z * z;
			fHeight = 8.0f * expf( -rsq / ( 2.0f * 3.0f ) );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "BillBoard_2_1" ),
