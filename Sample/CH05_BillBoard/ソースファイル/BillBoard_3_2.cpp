...
// ビルボードのZソート
int SortBillboard( MY_BILLBOARD *pSource, MY_BILLBOARD **ppSort,
				   int nDataNum, XMMATRIX *pmView )
{
	int					i, j;

	for ( i = 0; i < nDataNum; i++ ) {
		pSource[i].fSortKey = pmView->_13 * pSource[i].v3Pos.x +
							  pmView->_23 * pSource[i].v3Pos.y +
							  pmView->_33 * pSource[i].v3Pos.z;		// View座標でのz成分
	}
	for ( i = 0; i < nDataNum; i++ ) {
		for ( j = i - 1; j >= 0; j-- ) {
			if ( ppSort[j]->fSortKey < pSource[i].fSortKey ) {
				ppSort[j + 1] = ppSort[j];
			}
			else {
				break;
...
ID3D11BlendState		*g_pbsAlphaBlend;			// 加算ブレンド
...
HRESULT Render( void )
{
    ...
    g_pImmediateContext->OMSetBlendState( g_pbsAlphaBlend, NULL, 0xFFFFFFFF );	// 加算ブレンド
	MY_BILLBOARD *pSort[BILLBOARD_NUM];
	SortBillboard( Billboards, pSort, BILLBOARD_NUM, &mView );
	int			i;
	for ( i = 0; i < BILLBOARD_NUM; i++ ) {
		g_mmBillboard.v4AddColor = XMFLOAT4( pSort[i]->fMarkBright, 0.0f, 0.0f, 0.0f );
		g_mmBillboard.mMatrix = MakeBillboardMatrix( &mView, &( pSort[i]->v3Pos ) );	// ビルボード行列作成
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "BillBoard_3_2" ),
