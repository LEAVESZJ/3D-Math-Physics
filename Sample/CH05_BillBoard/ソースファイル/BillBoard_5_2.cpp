...
XMFLOAT3 Add( XMFLOAT3 *pv3Vec1, XMFLOAT3 *pv3Vec2 )						// ベクトルの引き算
{
	return XMFLOAT3( pv3Vec1->x + pv3Vec2->x,
					 pv3Vec1->y + pv3Vec2->y,
					 pv3Vec1->z + pv3Vec2->z );
...
// 地面頂点の法線ベクトル作成
// 法線ベクトルを補間するためには、各頂点について法線ベクトルが事前に必要
int MakeGroundNormals( CUSTOMVERTEX *pVertices, WORD *pIndices, int nIndexNum )
{
	...
	for ( i = 0; i < nIndexNum; i += 3 ) {
		...
		nIndex_X = nIndex1 % HEIGHT_NUM;  nIndex_Z = nIndex1 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
		nIndex_X = nIndex2 % HEIGHT_NUM;  nIndex_Z = nIndex2 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
		nIndex_X = nIndex3 % HEIGHT_NUM;  nIndex_Z = nIndex3 / HEIGHT_NUM;
		g_v3Normal[nIndex_Z][nIndex_X] = Add( &g_v3Normal[nIndex_Z][nIndex_X], &v3Normal );
	}
...
int MovePlayer( void )									// プレイヤーの移動
{
	...
	if ( GetAsyncKeyState( VK_UP ) ) {
		Player_1.v3Pos.x += PLAYER_SPEED * cosf( Player_1.fAngleXZ );
		Player_1.v3Pos.z += PLAYER_SPEED * sinf( Player_1.fAngleXZ );
	}
	// 手前
...
int Make3DRectangleIndexed( float fWidth, float fHeight, float fDepth, int bCenterMode,
							...
							int nIndexOffset )				// 直方体の作成(インデックス付き)
{
	int					i;
...
HRESULT InitGeometry( void )
{
	...
	hr = LoadTexture( _T( "Tri_Tex.bmp" ), &g_tSphere1Texture, 64, 64, 64, 64 );
    if ( FAILED( hr ) ) {
 		MessageBox( NULL, _T( "Can't open Tri_Tex.bmp" ), _T( "Error" ), MB_OK );
       return hr;
    }
	g_tSphere2Texture.pSRViewTexture =  NULL;
	hr = LoadTexture( _T( "13.bmp" ), &g_tSphere2Texture, 614, 614, 1024, 1024 );
    if ( FAILED( hr ) ) {
 		MessageBox( NULL, _T( "Can't open 13.bmp" ), _T( "Error" ), MB_OK );
       return hr;
    }
	...
	float			u, v;
	FILE				*fp;							// ファイルポインタ
	char				szLineBuf[200];					// ラインバッファ
	char				*pBufLoc;						// バッファ内位置
	
	// 標高ファイルオープン
	fopen_s( &fp, "G_Height.txt", "r" );
	if ( fp == NULL ) {
		MessageBox( NULL, _T( "Can't open G_Height.txt" ), _T( "Error" ), MB_OK );
		return E_FAIL;
	}
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		fgets( szLineBuf, 200, fp );					// １ライン読み出し
		pBufLoc = szLineBuf;
		x = -GROUND_SIZE / 2;
		v = ( float )i;
		for ( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ ) {
			y = ( float )atof( pBufLoc ) / 30.0f;				// 数値に変換
			g_fHeights[i][j] = y;
			...
			nIndex++;
			while ( !isdigit( *pBufLoc ) ) pBufLoc++;		// 次の数字へ
			while ( isdigit( *pBufLoc ) ) pBufLoc++;
			x += BLOCK_WIDTH;
		}
		z += BLOCK_WIDTH;
	}
...
HRESULT Render( void )
{
	...
	g_mmGround.v4AddColor = XMFLOAT4( -1.0f, -1.0f, -1.0f, 1.0f );
	DrawMyModelPartial( &g_mmGround, &mViewProjection, g_nHitTriangleIndex, 1 );
    g_pImmediateContext->OMSetRenderTargets( 1, &g_pRTV, g_pDepthStencilView );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "BillBoard_5_2" ),
