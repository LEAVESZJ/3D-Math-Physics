...
#define WEIGHT_NUM					11					// おもり数
#define CENTER_HEIGHT				2.0f				// 中立点高さ
#define SPRING_K					0.001f				// ばね定数
#define SPRING_K2					0.003f				// おもり間ばね定数
#define DIVIDE_T					10					// 時間分割精度
...
MY_WEIGHT	g_Weights[WEIGHT_NUM];						// おもりデータ
...
int InitPlayer( void )
{
	...
	fWeight_Base = -( WEIGHT_NUM - 1 ) / 2.0f;
	// おもり1
	g_Weights[0].v3Pos = XMFLOAT3( fWeight_Base, CENTER_HEIGHT, 0.0f );
	g_Weights[0].v3Vel = XMFLOAT3( 0.0f, 0.0f, 0.0f );
	for ( i = 1; i < WEIGHT_NUM - 1; i++ ) {
		g_Weights[i].v3Pos = XMFLOAT3( fWeight_Base + i, CENTER_HEIGHT, 0.0f );
		g_Weights[i].v3Vel = XMFLOAT3( 0.0f, 0.0f, 0.0f );
		if ( i < 5 ) g_Weights[i].v3Pos.y += ( 5 - i ) / 2.0f;
	}
	// おもり2
	g_Weights[WEIGHT_NUM - 1].v3Pos = XMFLOAT3( fWeight_Base + WEIGHT_NUM - 1, CENTER_HEIGHT, 0.0f );
	g_Weights[WEIGHT_NUM - 1].v3Vel = XMFLOAT3( 0.0f, 0.0f, 0.0f );
...
{
	int					i, j;
	float				fSpring_SF[WEIGHT_NUM];

	for ( i = 0; i < DIVIDE_T; i++ ) {
		for ( j = 0; j < WEIGHT_NUM - 1; j++ ) {
			fSpring_SF[j] = SPRING_K2 * ( g_Weights[j + 1].v3Pos.y - g_Weights[j].v3Pos.y );
		}
		for ( j = 1; j < WEIGHT_NUM - 1; j++ ) {
			g_Weights[j].v3Pos.y += g_Weights[j].v3Vel.y / DIVIDE_T;
			g_Weights[j].v3Vel.y += ( -SPRING_K * ( g_Weights[j].v3Pos.y - CENTER_HEIGHT )
									  - fSpring_SF[j - 1] + fSpring_SF[j] ) / DIVIDE_T;
		}
	}

	for ( j = 1; j < WEIGHT_NUM - 1; j++ ) {
		g_Weights[j].v3Vel.y *= 0.999f;
	}			
...
HRESULT InitGeometry( void )
{
	...
	Make3DRectangleIndexed( 1.0f, 0.7f, 1.0f, true,
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_4_3" ),
