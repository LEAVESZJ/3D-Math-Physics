...
float GetBarrelAngle( float fDistance, float fVel_0 )
{
	...
	if ( fSin > 1.0f ) fSin = 1.0f;					// 届かない場合の処理

	return 0.5f * asinf( fSin );
}


int InitTatget( void )									// ターゲットの初期化
{
	v3TargetPos.x = ( float )rand() * 10.0f / RAND_MAX - 5.0f;
	v3TargetPos.y = SHELL_RADIUS;
	v3TargetPos.z = ( float )rand() * 4.0f / RAND_MAX + 3.0f;

	return 0;
}

int InitShell( void )									// 砲弾の初期化
{
	// プレイヤー1
...
int MovePlayer( void )									// 砲塔の移動
{
	...
	if ( !( Shell_1.bActive ) ) {
		...
		Player_1.fTheta = GetBarrelAngle( fDistance, SHELL_FIRST_VEL );
		Shell_1.bActive = true;
		Shell_1.v3Pos = XMFLOAT3( 0.0f, SHELL_RADIUS, 0.0f );
		Shell_1.v3Vel = XMFLOAT3( SHELL_FIRST_VEL * cosf( Player_1.fTheta ) * cosf( -Player_1.fPhi ),
								  SHELL_FIRST_VEL * sinf( Player_1.fTheta ),
								  SHELL_FIRST_VEL * cosf( Player_1.fTheta ) * sinf( -Player_1.fPhi ) );
	}
...
//	int						i;
	...
	XMVECTOR At = XMVectorSet( 0.0f, 2.8f, 0.0f, 0.0f );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_2_2" ),
