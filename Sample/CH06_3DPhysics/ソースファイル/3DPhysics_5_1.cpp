...
#define GROUND_DIVIDE_NUM			30					// 床分割数
...
int MakeHeights( void )
{
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		...
		for ( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ ) {
			g_fHeights[i][j] = 3.0f * sinf( 2.0f * PI * x / GROUND_SIZE );
			x += BLOCK_WIDTH;
		}
		z += BLOCK_WIDTH;
	}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_5_1" ),
