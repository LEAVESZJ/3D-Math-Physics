...
#define PLAYER_SPEED				0.2f				// プレイヤーの移動速度
...
#define ROUND_TIME					2000				// 周期(ミリ秒単位)
#define WAVE_NUM					16					// 波の数
#define WAVE_HEIGHT					0.066f				// 波の高さ
#define WAVE_VELOCITY				0.003f				// 波の速さ
...
int InitWaves( void )
{
	...
	for ( i = 0; i < WAVE_NUM; i++ ) {
		Waves[i].fLength = rand() * 15.0f / RAND_MAX + 1.5f;	// 波長
		Waves[i].fAmp = WAVE_HEIGHT * Waves[i].fLength;			// 振幅	
		fAngle = 2.0f * PI * rand() / RAND_MAX;
		Waves[i].kx = cosf( fAngle ) / Waves[i].fLength;		// 波数ベクトル
		Waves[i].kz = sinf( fAngle ) / Waves[i].fLength;
		Waves[i].fDelta = 2.0f * PI * rand() / RAND_MAX;		// 位相
	}
...
int MakeHeights( void )
{
	...
	float				x, z;
	float				fTime;
	static DWORD		nFirstTime = timeGetTime();
	float				fFreq;

	// 時刻
	fTime = ( float )( timeGetTime() - nFirstTime );
	...
	for ( i = 0; i < GROUND_DIVIDE_NUM + 1; i++ ) {
		...
		for ( j = 0; j < GROUND_DIVIDE_NUM + 1; j++ ) {
			...
			for ( k = 0; k < WAVE_NUM; k++ ) {
				fFreq = WAVE_VELOCITY / Waves[k].fLength;
				g_fHeights[i][j] += Waves[k].fAmp * sinf( 2.0f * PI * ( fTime * fFreq
																	    - ( x * Waves[k].kx + z * Waves[k].kz ) )
														  + Waves[k].fDelta );
			}
			x += BLOCK_WIDTH;
		}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DPhysics_6_2" ),
