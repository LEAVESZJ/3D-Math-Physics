...
float CheckGroundHeight( MY_PLAYER *pPlayer )
{
	...
	if ( ( fBlockRes_x + fBlockRes_z ) < 1.0f ) {
		fGrad_x = ( g_fHeights[nBlockIndex_z][nBlockIndex_x + 1]
				  - g_fHeights[nBlockIndex_z][nBlockIndex_x] );
		fGrad_z = ( g_fHeights[nBlockIndex_z + 1][nBlockIndex_x]
				  - g_fHeights[nBlockIndex_z][nBlockIndex_x] );
		fHeight = fGrad_x * fBlockRes_x + fGrad_z * fBlockRes_z
				  + g_fHeights[nBlockIndex_z][nBlockIndex_x];
		nInIndex = 0;
	}
	else {
		fGrad_x = ( g_fHeights[nBlockIndex_z + 1][nBlockIndex_x]
				  - g_fHeights[nBlockIndex_z + 1][nBlockIndex_x + 1] );
		fGrad_z = ( g_fHeights[nBlockIndex_z][nBlockIndex_x + 1]
				  - g_fHeights[nBlockIndex_z + 1][nBlockIndex_x + 1] );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_5_2" ),
