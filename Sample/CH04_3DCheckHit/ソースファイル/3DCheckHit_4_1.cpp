...
#define HEIGHT_NUM				11								// 高さデータ数
#define BLOCK_NUM				( HEIGHT_NUM - 1 )				// 地形ブロック数
...
float CheckGroundHeight( MY_PLAYER *pPlayer )
{
	...
	fPlayerBlock = ( pPlayer->v3Pos.x + ( GROUND_SIZE / 2 ) ) / ( float )BLOCK_WIDTH;
	nBlockIndex = ( int )fPlayerBlock;
	if ( nBlockIndex < 0 ) nBlockIndex = 0;
	if ( nBlockIndex > HEIGHT_NUM - 2 ) nBlockIndex = HEIGHT_NUM - 2;
	fBlockRes = fPlayerBlock - nBlockIndex;
	fGrad = ( g_fHeights[nBlockIndex + 1] - g_fHeights[nBlockIndex] );
...
HRESULT InitGeometry( void )
{
	...
	for ( i = 0; i < HEIGHT_NUM - 1; i++ ) {
		g_wIndices[g_nIndexNum + nIndex    ] = i * 2;
...
//    g_pImmediateContext->RSSetState( g_pRS_Cull_CW );				// カリングあり
    g_pImmediateContext->RSSetState( g_pRS );				// カリングあり
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_4_1" ),
