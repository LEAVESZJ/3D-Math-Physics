...
float Dot( XMFLOAT3 *pv3Vec1, XMFLOAT3 *pv3Vec2 )						// ベクトルの内積
{
...
int CheckHit( XMFLOAT3 *pv3Triangle, XMFLOAT3 *pv3Point )
{
	...
	if ( bHit == TRUE ) {
		// プレイヤー位置制御
		if ( ( fabsf( fCross0 ) <= fabsf( fCross1 ) ) &&
			 ( fabsf( fCross0 ) <= fabsf( fCross2 ) ) )
		{
			// 辺０に一番近い
			...
			if ( fabsf( fCross1 ) <= fabsf( fCross2 ) ) {
				// 辺１に一番近い
...
HRESULT InitGeometry( void )
{
    ...
    if ( FAILED( hr ) ) {
 		MessageBox( NULL, _T( "Can't open 8_2.bmp" ), _T( "Error" ), MB_OK );
       return hr;
    }
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "3DCheckHit_2_2" ),
