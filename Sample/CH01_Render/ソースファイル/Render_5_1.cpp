...
int RenderScanLine( void )
{
	...
	vBase1_x = cosf( fAngle );						// 基底ベクトル1
	vBase1_y = sinf( fAngle );
	vBase2_x = cosf( fAngle + PI / 2.0f );			// 基底ベクトル2
	vBase2_y = sinf( fAngle + PI / 2.0f );
	bx = -VIEW_WIDTH  / 2.0f * vBase1_x + -VIEW_HEIGHT / 2.0f * vBase2_x; 
	by = -VIEW_WIDTH  / 2.0f * vBase1_y + -VIEW_HEIGHT / 2.0f * vBase2_y; 
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y方向ループ
		cx = bx; 
		cy = by; 
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			GetBMPPixel( ( int )( cx + VIEW_WIDTH  / 2.0f ),
						 ( int )( cy + VIEW_HEIGHT / 2.0f ),
						 &mbPicture,
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Render_5_1" ),
