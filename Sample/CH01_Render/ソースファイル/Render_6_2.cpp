...
// バイリニアフィルタを掛ける
int FilterBiLinear( int nBright1, int nBright2, int nBright3, int nBright4,
				    float lu, float lv )
{
	float			fBright_x1, fBright_x2;

	fBright_x1 = nBright1 * ( 1.0f - lu ) + nBright2 * lu;
	fBright_x2 = nBright3 * ( 1.0f - lu ) + nBright4 * lu;

	return ( int )( fBright_x1 * ( 1.0f - lv ) + fBright_x2 * lv );
}

// バイリニアフィルタ付きピクセルの取得
int GetPixelFiltered( float x, float y, MemoryBMP *pmbSrc,
					  int *pnRed, int *pnGreen, int *pnBlue )
{
	float				lu, lv;
	...
	GetBMPPixel( ( int )x,     ( int )y,     pmbSrc,
				 &nRed1, &nGreen1, &nBlue1 );			// 点1取得
	GetBMPPixel( ( int )x + 1, ( int )y,     pmbSrc,
				 &nRed2, &nGreen2, &nBlue2 );			// 点2取得
	GetBMPPixel( ( int )x,     ( int )y + 1, pmbSrc,
				 &nRed3, &nGreen3, &nBlue3 );			// 点3取得
	GetBMPPixel( ( int )x + 1, ( int )y + 1, pmbSrc,
				 &nRed4, &nGreen4, &nBlue4 );			// 点4取得

	lu = x - ( int )x;  lv = y - ( int )y;
...
int RenderScanLine( void )
{
	...
	for ( y = 0; y < VIEW_HEIGHT; y++ ) {		// y方向ループ
		...
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			GetPixelFiltered( cx + VIEW_WIDTH  / 2.0f,
							  cy + VIEW_HEIGHT / 4.0f,
							  &mbPicture,
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Render_6_2" ),
