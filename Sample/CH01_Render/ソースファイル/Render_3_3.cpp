		...
		for ( x = 0; x < VIEW_WIDTH; x++ ) {	// x方向ループ
			...
			if ( dzsq > 0.0f ) {
				...
				if ( ( int )( fAngle / ( 2.0f * PI ) * 16 ) & 1 ) {
					...
					nRed =   ( int )( nBright * 0.7f );
					nGreen = ( int )( nBright * 0.9f );
					nBlue =  ( int )( nBright * 0.3f );
					DrawPoints( x, y, nRed ,nGreen, nBlue );		// 中間色
				}
			}
		}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Render_3_3" ),
