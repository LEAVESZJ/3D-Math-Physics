...
{
	int					i, j;
	CUSTOMVERTEX		Vertices[( CORNER_NUM + 1 ) * ( CORNER_NUM / 2 + 1 )];	// 頂点データ
	WORD				wIndices[CORNER_NUM * CORNER_NUM / 2 * 2 * 3];	// インデックスデータ
	float				fTheta;
	float				fPhi;
	float				fAngleDelta;
	float				x, y, z;
...
{
    XMMATRIX	matView;					// 変換行列
    XMMATRIX	matWorld;					// ワールド行列
	XMFLOAT3	v3Light;					// 光源方向ベクトル
	float		fAmbient;					// アンビエント光強度
	float		fDirectional;				// 平行光強度
};
...
HRESULT MakeShaders( void )
{
...
#endif
    // コンパイル
    hr = D3DX11CompileFromFile( _T( "Basic_3D_Light.fx" ), NULL, NULL, "VS", "vs_4_0_level_9_1",
    ...
    hr = D3DX11CompileFromFile( _T( "Basic_3D_Light.fx" ), NULL, NULL, "PS", "ps_4_0_level_9_1",
...
HRESULT InitGeometry( void )
{
	...
	//hr = LoadTexture( _T( "FloorTexture.bmp" ), &g_tTexture, 640, 480, 1024, 512 );
 //   if ( FAILED( hr ) ) {
 //		MessageBox( NULL, _T( "Can't open FloorTexture.bmp" ), _T( "Error" ), MB_OK );
 //      return hr;
 //   }
...
HRESULT Render( void )
{
	...
	matProjection = XMMatrixPerspectiveFovLH( XM_PIDIV4, VIEW_WIDTH / ( FLOAT )VIEW_HEIGHT, 0.01f, 100.0f );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Textures_3_1" ),
