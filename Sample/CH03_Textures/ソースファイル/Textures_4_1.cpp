...
{
	int					i, j;
	CUSTOMVERTEX		Vertices[2 * ( CORNER_NUM + 1 )];	// 頂点データ
	WORD				wIndices[CORNER_NUM * 2 * 3];	// インデックスデータ
	float				fTheta;
	float				fAngleDelta;
	...
	for ( i = 0; i < 2; i++ ) {
		...
		for ( j = 0; j < CORNER_NUM + 1; j++ ) {
			Vertices[nIndex].v4Pos  = XMFLOAT4( R * cosf( fTheta ),
												R * sinf( fTheta ),
												CYLINDER_LENGTH * i, 1.0f );
	...
	for ( i = 0; i < CORNER_NUM; i++ ) {
		...
		wIndices[nIndex    ] = i + 1;
		wIndices[nIndex + 1] = ( CORNER_NUM + 1 ) + i;
		wIndices[nIndex + 2] = ( CORNER_NUM + 1 ) + i + 1;
		nIndex += 3;
	}

	// 頂点インデックスを使って描画
	DrawIndexed3DPolygonsTex( Vertices, 2 * ( CORNER_NUM + 1 ),
							  wIndices, CORNER_NUM * 2 * 3 );
...
HRESULT Render( void )
{
	...
	mProjection = XMMatrixPerspectiveFovLH( XM_PIDIV4, VIEW_WIDTH / ( FLOAT )VIEW_HEIGHT, 0.01f, 1000.0f );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Textures_4_1" ),
