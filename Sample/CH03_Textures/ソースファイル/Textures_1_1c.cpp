...
#define OBJECT_SIZE			1.5f				// 物体のサイズ
#define SCALE_SPEED			0.02f				// テクスチャ座標変化速度
...
int DrawChangingPictures( void )						// 画像の描画
{
	...
	WORD					wIndices[6] = { 0, 1, 2,
											2, 1, 3, };	// インデックスデータ
	static float			fTexScale_x = 0.0f;	// テクスチャ座標大きさ
	static float			fTexScale_y = 0.0f;	// テクスチャ座標大きさ
	...
	if ( GetAsyncKeyState( VK_RIGHT ) ) {
		fTexScale_x += SCALE_SPEED;
	}
	if ( GetAsyncKeyState( VK_LEFT ) ) {
		fTexScale_x -= SCALE_SPEED;
	}
	if ( GetAsyncKeyState( VK_DOWN ) ) {
		fTexScale_y += SCALE_SPEED;
	}
	if ( GetAsyncKeyState( VK_UP ) ) {
		fTexScale_y -= SCALE_SPEED;
	}
	Vertices[0].v2UV = XMFLOAT2( fTexScale_x,        fTexScale_y );
	Vertices[1].v2UV = XMFLOAT2( fTexScale_x + 1.0f, fTexScale_y );
	Vertices[2].v2UV = XMFLOAT2( fTexScale_x,        fTexScale_y + 1.0f );
	Vertices[3].v2UV = XMFLOAT2( fTexScale_x + 1.0f, fTexScale_y + 1.0f );
...
int InitDrawModes( void )
{
    ...
    samDesc.AddressU        = D3D11_TEXTURE_ADDRESS_BORDER; //D3D11_TEXTURE_ADDRESS_WRAP;
    samDesc.AddressV        = D3D11_TEXTURE_ADDRESS_BORDER; //D3D11_TEXTURE_ADDRESS_WRAP;
    samDesc.AddressW        = D3D11_TEXTURE_ADDRESS_WRAP;
    samDesc.ComparisonFunc  = D3D11_COMPARISON_ALWAYS;
	float v4Color[4] = { 1.0f, 0.0f, 0.0f, 0.0f };			// ボーダーカラー
	memcpy( samDesc.BorderColor, v4Color, sizeof( float ) * 4 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Textures_1_1c" ),
