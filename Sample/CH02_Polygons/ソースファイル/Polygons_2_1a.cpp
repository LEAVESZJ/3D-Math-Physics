...
{
	float			fAngle;
	XMMATRIX		xmResult;							// 結果行列
	static float	x = 0.0f;							// x座標

	fAngle = 2.0f * 3.1415f * ( float )( timeGetTime() % 2000 ) / 2000.0f;
	xmResult = XMMatrixRotationY( fAngle );
	if ( GetAsyncKeyState( VK_LEFT  ) ) x -= POLY_SPEED;	// 左
	if ( GetAsyncKeyState( VK_RIGHT ) ) x += POLY_SPEED;	// 右
	xmResult._41 = x;
	return xmResult;
}
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_2_1a" ),
