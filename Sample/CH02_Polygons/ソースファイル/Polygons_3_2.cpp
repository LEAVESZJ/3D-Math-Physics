...
int DrawChangingPictures( void )
{
	...
	fAngleDelta = 2.0f * PI / 4.0f;
	fAngle1 = 0.0f;
	fAngle2 = fAngleDelta;
	for ( i = 0; i < 4; i++ ) {
		Draw3DPolygon( R * cosf( fAngle1 ), -1.0f,  R * sinf( fAngle1 ), 0xffff0000,
					   R * cosf( fAngle2 ), -1.0f,  R * sinf( fAngle2 ), 0xff00ff00,
									  0.0f,  1.0f,                 0.0f, 0xff0000ff );
		Draw3DPolygon( R * cosf( fAngle2 ), -1.0f,  R * sinf( fAngle2 ), 0xffff0000,
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_3_2" ),
