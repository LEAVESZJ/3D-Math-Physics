...
{
	int						i;
	CUSTOMVERTEX			Vertices[CORNER_NUM + 1];	// 頂点データ
	WORD					wIndices[CORNER_NUM * 3];	// インデックスデータ
	float					fAngle;
	float					fAngleDelta;
	...
	// インデックスデータ作成
	nIndex = 0;
	for ( i = 1; i <= CORNER_NUM; i++ ) {
		wIndices[nIndex    ] = 0;
		wIndices[nIndex + 1] = i;
		wIndices[nIndex + 2] = ( i % CORNER_NUM ) + 1;
		nIndex += 3;
	}

	// 頂点インデックスを使って描画
	DrawIndexed3DPolygons( Vertices, CORNER_NUM + 1, wIndices, CORNER_NUM * 3 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_6_2a" ),
