...
// 画像の描画
// 頂点インデックスを使う場合、頂点を置く部分は単純になる
// 単純に、ある緯度・経度部分(緯線と経線の交点部分)に頂点を置いていけばよい
// ポリゴン形成の複雑さは、頂点インデックス生成部分に移る
int DrawChangingPictures( void )
{
	int					i, j;
	CUSTOMVERTEX		Vertices[CORNER_NUM * ( CORNER_NUM / 2 + 1 )];	// 頂点データ
	...
	// 頂点インデックスを使って描画
	DrawIndexed3DPolygons( Vertices, CORNER_NUM * ( CORNER_NUM / 2 + 1 ),
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_6_3" ),
