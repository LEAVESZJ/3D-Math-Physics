...
int DrawChangingPictures( void )
{
	...
	for ( i = 0; i < 5 - 1; i++ ) {
		...
		for ( j = 0; j < CORNER_NUM; j++ ) {
			p1 = RotateXYOnXZ( SrcPoints[i][0], SrcPoints[i][1], fTheta1 );
			p2 = RotateXYOnXZ( SrcPoints[i][0], SrcPoints[i][1], fTheta2 );
			p3 = RotateXYOnXZ( SrcPoints[i + 1][0], SrcPoints[i + 1][1], fTheta1 );
			p4 = RotateXYOnXZ( SrcPoints[i + 1][0], SrcPoints[i + 1][1], fTheta2 );
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_5_1" ),
