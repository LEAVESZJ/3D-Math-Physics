...
{
	int					x, y;
	float				x1 = 200.0f, y1 = 50.0f;
	float				x2 = 50.0f, y2 = 300.0f;
	float				x3 = 500.0f, y3 = 200.0f;
	float				fTemp;
	float				a12, a13, a23;
	float				xl, xr;
	...
	if ( a12 < a13 ) {
		for ( y = ( int )y1; y < ( int )y2; y++ ) {		// y方向ループ
				for ( x = ( int )xl; x < ( int )xr; x++ ) {	// x方向ループ
					DrawPoints( x, y, 255, 255, 255 );
				}
			xl += a12;  xr += a13;
			FlushDrawingPictures();
		}
		for ( y = ( int )y2; y < ( int )y3; y++ ) {		// y方向ループ
				for ( x = ( int )xl; x < ( int )xr; x++ ) {	// x方向ループ
					DrawPoints( x, y, 255, 255, 255 );
				}
			xl += a23;  xr += a13;
...
int WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR, int )
{
    ...
    g_hWnd = CreateWindow( _T( "D3D Sample" ), _T( "Polygons_1_1" ),
